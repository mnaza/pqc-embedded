//! ML-DSA-65 verification, sized the same way as the LMS probe.
//!
...
pub extern "C" fn _start() -> ! {
    let ok = unsafe {
        ...
        let enc_pk = EncodedVerifyingKey::<MlDsa65>::try_from(pk).unwrap();
        let enc_sig = EncodedSignature::<MlDsa65>::try_from(sigbytes).unwrap();
        let vk = VerifyingKey::<MlDsa65>::decode(&enc_pk);
        match Signature::<MlDsa65>::decode(&enc_sig) {
